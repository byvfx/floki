...
### Fixed
- The OCIO CPU display transform now shows nothing (rather than wrong colors)
  when the transform fails: previously the untransformed scene-linear buffer was
  clamped to [0,1] and displayed, silently presenting incorrect color with no
  indication the transform never ran.
...
- Internal: hardened a set of panic-prone `unwrap()`s across the GPU/CPU canvas
  ...
  behavior change in the normal case — the app just degrades gracefully instead
  of crashing if an invariant is ever violated.
